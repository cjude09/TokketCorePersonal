...
{
    [Activity(Label = "Opportunity Info", Theme = "@style/AppTheme", ConfigurationChanges = ConfigChanges.ScreenSize | ConfigChanges.UiMode | ConfigChanges.Orientation, ScreenOrientation = ScreenOrientation.Portrait)]
    public class OpportunityInfoActivity : BaseActivity, View.IOnTouchListener
    {
        internal static OpportunityInfoActivity Instance;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.custominfo_page);
            ...
            BtnRatingComment.Click += async (sender, e) => {
                await OnClickAddReaction(BtnRatingComment);
            };
            ...
            RatingButton.Click += delegate
            {
                if (RatingLinearComment.Visibility == ViewStates.Visible)
                {
                    RatingLinearComment.Visibility = ViewStates.Gone;
                    NestedComment.Visibility = ViewStates.Visible;
                }
                else
                {
                    RatingLinearComment.Visibility = ViewStates.Visible;
                    GemsParentContainer.Visibility = ViewStates.Visible;
                    var inputManager = (InputMethodManager)GetSystemService(InputMethodService);
                    EditRatingText.RequestFocus();
                    inputManager.ShowSoftInput(EditRatingText, 0);
                    NestedComment.Visibility = ViewStates.Gone;
                }

            };
            EditRatingText.FocusChange += delegate
            {
                if (EditRatingText.IsFocused)
                {
                    //BtnSmileInaccurate.SetImageResource(Resource.Drawable.TOKKET_smiley_1B);
                    //ExpandedTokMojiRating.Expanded = false;
                }
                else
                {
                    RatingLinearComment.Visibility = ViewStates.Gone;
            ...
            classTokModel = JsonConvert.DeserializeObject<OpportunityTok>(Intent.GetStringExtra("opportunityTok"));
            //PrimaryFieldText.Text = classTokModel.PrimaryFieldText;
            OpportunityTitle.Text = classTokModel.PrimaryFieldText;
            OpportunityTitle.PaintFlags = PaintFlags.UnderlineText;
            Title = string.IsNullOrEmpty(classTokModel.PrimaryFieldText) ?  "Info": classTokModel.PrimaryFieldText;
            if (!string.IsNullOrEmpty(classTokModel.Type) && string.IsNullOrEmpty(classTokModel.OpportunityType))
                IsTraining = true;
            try
            {
            ...
            NestedComment.LayoutChange += delegate
            {
                ...
                if (heightDifference > 500 || RecyclerTokMojis.Visibility == ViewStates.Gone) //189
                {
                    GemsParentContainer.Visibility = ViewStates.Visible;
                }
            };
        ...
        private void FillUpFields()
        {
            ...
            if (!IsTraining) {
                CustomLinear.Visibility = ViewStates.Visible;
                InfoTrainingLinear.Visibility = ViewStates.Gone;
                OpportunityType.Text = "Type: " + classTokModel.OpportunityType;
                CompanyName.Text = "Company Name: " + classTokModel.SecondaryFieldText;
                PayAmount.Text = "Pay Amount: " + classTokModel.Amount;
                DueDate.Text = "Due Date: " + classTokModel.ApplicationDeadline;
                Description.Text = "Description: " + classTokModel.Description;
                Requirements.Text = "Requirements: " + classTokModel.Requirements;
                Email.Text = "Requirements: " + classTokModel.Email;
                Address.Text = "Address: " + classTokModel.HomeAddress;
                PhoneNumber.Text = "Phone Number: " + classTokModel.PhoneNumber;
                AboutTheCompany.Text = "About the company: " + classTokModel.AboutCompany;
            ...
            {
                InfoTrainingLinear.RemoveAllViews();
                CustomLinear.Visibility = ViewStates.Gone;
                InfoTrainingLinear.Visibility = ViewStates.Visible;
                tokcategory.Text = classTokModel.Category;
                InfoTrainingLinear.AddView(ItemText($"Type: {classTokModel.Type}"));
                switch (classTokModel.Type) {
                    case "course":
                        if(!string.IsNullOrEmpty(classTokModel.School))
                          InfoTrainingLinear.AddView(ItemText($"School: {classTokModel.School}"));
                        if (!string.IsNullOrEmpty(classTokModel.PrimaryFieldText))
                            InfoTrainingLinear.AddView(ItemText($"Course Name: {classTokModel.PrimaryFieldText}"));
                        if (!string.IsNullOrEmpty(classTokModel.Description))
                            InfoTrainingLinear.AddView(ItemText($"Description: {classTokModel.Description}"));
                        if (!string.IsNullOrEmpty(classTokModel.About))
                            InfoTrainingLinear.AddView(ItemText($"About: {classTokModel.About}"));
                        if (!string.IsNullOrEmpty(classTokModel.HowItWorks))
                            InfoTrainingLinear.AddView(ItemText($"How it works: {classTokModel.HowItWorks}"));
                        if (!string.IsNullOrEmpty(classTokModel.Instructors))
                            InfoTrainingLinear.AddView(ItemText($"Instructors: {classTokModel.Instructors}"));
                        if (!string.IsNullOrEmpty(classTokModel.EnrollmentOptions))
                            InfoTrainingLinear.AddView(ItemText($"Enrollment Options: {classTokModel.EnrollmentOptions}"));
                        if (!string.IsNullOrEmpty(classTokModel.SourceLink))
                            InfoTrainingLinear.AddView(ItemText($"Source Link: {classTokModel.SourceLink}"));
                        if (!string.IsNullOrEmpty(classTokModel.Enrolled.ToString()))
                            InfoTrainingLinear.AddView(ItemText($"People Enrolled: {classTokModel.Enrolled.ToString()}"));
                        if (!string.IsNullOrEmpty(classTokModel.ItemCountry))
                            InfoTrainingLinear.AddView(ItemText($"Country: {classTokModel.ItemCountry}"));
                        if (!string.IsNullOrEmpty(classTokModel.ItemState))
                            InfoTrainingLinear.AddView(ItemText($"State: {CountryTool.GetStateName(classTokModel.ItemState)}"));
                        if (!string.IsNullOrEmpty(classTokModel.City))
                            InfoTrainingLinear.AddView(ItemText($"City: {classTokModel.City}"));
                        if (!string.IsNullOrEmpty(classTokModel.FAQ))
                            InfoTrainingLinear.AddView(ItemText($"FAQ: {classTokModel.FAQ}"));
                        if (!string.IsNullOrEmpty(classTokModel.Days))
                            InfoTrainingLinear.AddView(ItemText($"Days: {classTokModel.Days}"));
                        if (!string.IsNullOrEmpty(classTokModel.StartTime))
                            InfoTrainingLinear.AddView(ItemText($"Start Time: {classTokModel.StartTime}"));
                        if (!string.IsNullOrEmpty(classTokModel.EndTime))
                            InfoTrainingLinear.AddView(ItemText($"End Time: {classTokModel.EndTime}"));
                        if (!string.IsNullOrEmpty(classTokModel.Dates))
                            InfoTrainingLinear.AddView(ItemText($"Dates: {classTokModel.Dates}"));
                        if (!string.IsNullOrEmpty(classTokModel.Units))
                            InfoTrainingLinear.AddView(ItemText($"Units: {classTokModel.Units}"));
                        if (!string.IsNullOrEmpty(classTokModel.EnrollmentRequirements))
                            InfoTrainingLinear.AddView(ItemText($"Enrollment Requirements: {classTokModel.EnrollmentRequirements}"));
                        if (!string.IsNullOrEmpty(classTokModel.ReservationRequirements))
                            InfoTrainingLinear.AddView(ItemText($"Reservation: {classTokModel.ReservationRequirements}"));
                        if (!string.IsNullOrEmpty(classTokModel.CourseMaterials))
                            InfoTrainingLinear.AddView(ItemText($"Course Materials: {classTokModel.CourseMaterials}"));
                        break;
                    case "program":
                        if (!string.IsNullOrEmpty(classTokModel.PrimaryFieldText))
                            InfoTrainingLinear.AddView(ItemText($"Program Name: {classTokModel.PrimaryFieldText}"));
                        if (!string.IsNullOrEmpty(classTokModel.Cost))
                            InfoTrainingLinear.AddView(ItemText($"Cost: {classTokModel.Cost}"));
                        if (!string.IsNullOrEmpty(classTokModel.Schedule))
                            InfoTrainingLinear.AddView(ItemText($"Schedule: {classTokModel.Schedule}"));
                        if (!string.IsNullOrEmpty(classTokModel.Level))
                            InfoTrainingLinear.AddView(ItemText($"Level: {classTokModel.Level}"));
                        if (!string.IsNullOrEmpty(classTokModel.Description))
                            InfoTrainingLinear.AddView(ItemText($"Description: {classTokModel.Description}"));
                        if (!string.IsNullOrEmpty(classTokModel.Duration))
                            InfoTrainingLinear.AddView(ItemText($"Duration: {classTokModel.Duration}"));
                        if (!string.IsNullOrEmpty(classTokModel.RelatedSkills))
                            InfoTrainingLinear.AddView(ItemText($"Related Skills and Certification Links: {classTokModel.RelatedSkills}"));
                        if (!string.IsNullOrEmpty(classTokModel.School))
                            InfoTrainingLinear.AddView(ItemText($"School: {classTokModel.School}"));
                        if (!string.IsNullOrEmpty(classTokModel.ItemCountry))
                            InfoTrainingLinear.AddView(ItemText($"Country:{classTokModel.ItemCountry}"));
                        ...
                        if (!string.IsNullOrEmpty(classTokModel.City))
                            InfoTrainingLinear.AddView(ItemText($"City: {classTokModel.City}"));
                        if (!string.IsNullOrEmpty(classTokModel.Enrolled.ToString()))
                            InfoTrainingLinear.AddView(ItemText($"Enrolled: {classTokModel.Enrolled.ToString()}"));
                        if (!string.IsNullOrEmpty(classTokModel.Projects.ToString()))
                            InfoTrainingLinear.AddView(ItemText($"# of Projects: {classTokModel.Projects.ToString()}"));
                        if (!string.IsNullOrEmpty(classTokModel.Assigments.ToString()))
                            InfoTrainingLinear.AddView(ItemText($"# of Assignments: {classTokModel.Assigments}"));
                        if (!string.IsNullOrEmpty(classTokModel.InstructorInfo))
                            InfoTrainingLinear.AddView(ItemText($"Instructor Info: {classTokModel.InstructorInfo}"));
                        if (!string.IsNullOrEmpty(classTokModel.OtherDetail?[0]))
                            InfoTrainingLinear.AddView(ItemText($"Detail: {classTokModel.OtherDetail[0]}"));
                        break;
                    case "tutor":
                        if (!string.IsNullOrEmpty(classTokModel.PrimaryFieldText))
                            InfoTrainingLinear.AddView(ItemText($"Tutor Name: {classTokModel.PrimaryFieldText}"));
                        if (!string.IsNullOrEmpty(classTokModel.Name))
                            InfoTrainingLinear.AddView(ItemText($"Education: {classTokModel.Name}"));
                        if (!string.IsNullOrEmpty(classTokModel.Experience))
                            InfoTrainingLinear.AddView(ItemText($"Experience: {classTokModel.Experience}"));
                        if (!string.IsNullOrEmpty(classTokModel.TutorDescription1))
                            InfoTrainingLinear.AddView(ItemText($"Tutor Description: {classTokModel.TutorDescription1}"));
                        if (!string.IsNullOrEmpty(classTokModel.ItemCountry))
                            InfoTrainingLinear.AddView(ItemText($"Country: {classTokModel.ItemCountry}"));
                        if (!string.IsNullOrEmpty(classTokModel.ItemState))
                            InfoTrainingLinear.AddView(ItemText($"State:  {CountryTool.GetStateName(classTokModel.ItemState)}"));
        ...
        private TextView ItemText(string text) {
            var view = new TextView(this) { LayoutParameters = new ViewGroup.LayoutParams(LayoutParams.MatchParent,LayoutParams.WrapContent) };
        ...
        private async Task LoadComments(string continuationtoken = "", bool isRefresh = false)
        {
            ...
            tasksList.Add(TokInfoVm.LoadRatings(classTokModel.Id, continuationtoken));
            ...
            adapterComments = TokInfoVm.CommentsCollection.GetRecyclerAdapter(BindCommentsViewHolder, Resource.Layout.opportunity_comments_row);
            RecyclerComments.SetAdapter(adapterComments);
            ...
            #endregion
            RatingLinearView.Visibility = ViewStates.Visible;
            RatingReview.Rating = (float)TokInfoVm.StarCountAverage;
            lblStarCount.Text = $"{Math.Round(TokInfoVm.StarCountAverage,2)} stars out of 5";


            //var ratings =  await TokInfoVm.LoadRatings(classTokModel.Id, continuationtoken);
            //  if (ratings.Count > 0)
            //  {
            //      var currentRating = ratings[0];
            //      RatingLinearView.Visibility = ViewStates.Visible;
            //      if (currentRating.StarRatingCount.HasValue)
            //      {
            //          RatingReview.Rating = (float)currentRating.StarRatingCount.Value;
            //          lblStarCount.Text = $"{currentRating.StarRatingCount.Value} stars out 5";
            //          lblRatingComment.Text = currentRating.Text;
            //      }
            //      else {
            //          RatingLinearView.Visibility = ViewStates.Gone;
            //      }
        ...
        private void BindCommentsViewHolder(CachingViewHolder holder, ReactionModel comment, int position)
        {
            ...
            var ratingComment = holder.FindCachedViewById<RatingBar>(Resource.Id.ratingreview);
            var lblRatingCount = holder.FindCachedViewById<TextView>(Resource.Id.lblStarCount);
            ...
            if (comment.StarRatingCount.HasValue)
            {
                ratingComment.Visibility = ViewStates.Visible;
                ratingComment.Rating = (float)comment.StarRatingCount;
                lblRatingCount.Text =  $"{comment.StarRatingCount.Value} stars out of 5";
            }
            else
            {
                ratingComment.Visibility = ViewStates.Gone;
            }
        ...
        private async Task LoadSelectedGems(string reaction = "All", string Id = "", long Index = 0)
        {
            ...
            foreach (var item in resultDataUserReaction.Results)
            {
                ...
                else
                {
                    ...
                    reactionQueryValues.item_id = classTokModel.Id;
                    ...
                    if (kind == "accurate")
                    {
                        ...
                        btnTotalReaction.Text = ((long)result.Count * 5).ToKMB();
                    }
                    else if (kind == "inaccurate")
                    {
                        isInaccurateAdded = true;
                        ...
                        btnTotalReaction.Text = Math.Abs((long)result.Count * -10).ToKMB();
                        reactionValueVM.Inaccurate = reactionValueVM.Inaccurate + 1;
                    }
        ...
        private async Task OnClickAddReaction(View v, int detailPosition = -1)
        {
            try
            {
                ...
                else
                {
                    ...
                    else if (kind == "comment")
                    {
                    ...
                    else if (kind == "star_rating") {
                        txtProgressText.Text = "Adding a rating...";
                        comment = EditRatingText.Text;
                        EditRatingText.Text = "";
                        ...
                        tokkepediaReaction.Timestamp = DateTime.Now;
                        tokkepediaReaction.IsComment = true;
                        tokkepediaReaction.StarRatingCount = (double)RatingComment.Rating;
                        tokkepediaReaction.StarRating = (double)RatingComment.Rating;
                        tokkepediaReaction.StarRatingsCount = (double)RatingComment.Rating;
                        RatingComment.Rating = 3.5f;
                        RatingLinearComment.Visibility = ViewStates.Gone;
                        //await TokInfoVm.LoadRatings(classTokModel.Id, "");
                        ////TokInfoVm.CommentsCollection.Add(tokkepediaReaction);
                        //adapterComments = TokInfoVm.CommentsCollection.GetRecyclerAdapter(BindCommentsViewHolder, Resource.Layout.opportunity_comments_row);
                ...
                if (isAddReaction)
                {
                    ...
                    if (result.ResultEnum == Shared.Helpers.Result.Success)
                    {
                        ...
                        else if (kind.ToLower() == "star_rating") {
                            await LoadComments();
                        }
                        else
                        {
        ...
        public override bool OnOptionsItemSelected(IMenuItem item)
        {
            switch (item.ItemId)
            {
                ...
                case Resource.Id.edit_yearbook:
                    if (!IsTraining)
                    {
                        var intent = new Intent(this, typeof(AddOpportunityActivity));
                        intent.PutExtra("isEdit", "true");
                        intent.PutExtra("opportunityModel", JsonConvert.SerializeObject(classTokModel));
                        StartActivityForResult(intent, REQUEST_PEERBOK_UPDATE);
                    }
                    else {
                        var intent = new Intent(this, typeof(AddTrainingActivity));
                        intent.PutExtra("isEdit", "true");
                        intent.PutExtra("trainingModel", JsonConvert.SerializeObject(classTokModel));
                        StartActivityForResult(intent, REQUEST_PEERBOK_UPDATE);
                    }
                ...
                case Resource.Id.delete_yearbook:
                    var objBuilder = new AlertDialog.Builder(this);
                    objBuilder.SetTitle("");
                    objBuilder.SetMessage("Do you wanna delete this Opportunity?");
                    objBuilder.SetCancelable(false);
                    ...
                    objDialog.SetButton((int)(DialogButtonType.Positive), "OK", async (s, ev) => {
                        ...
                        {
                            if (!IsTraining)
                                OpportunityListActivity.Instance.RemoveOpportunity(classTokModel);
                            else
                                TrainingListActivity.Instance.RemoveOpportunity(classTokModel);
                            Finish();
                        }
                        linearProgress.Visibility = ViewStates.Gone;
        ...
        protected override void OnActivityResult(int requestCode, [GeneratedEnum] AppResult resultCode, Intent data)
        {
            ...
            if ((requestCode == REQUEST_TOK_INFO_REPLY) && (resultCode == AppResult.Ok))
            {
            ...
            else if ((requestCode == REQUEST_PEERBOK_UPDATE) && (resultCode == AppResult.Ok))
            {
                var updatedData = data.GetStringExtra("updatedOpportunity");
                classTokModel = JsonConvert.DeserializeObject<OpportunityTok>(updatedData);
                FillUpFields();
            }
        }
        ...
        public TextView OpportunityTitle => FindViewById<TextView>(Resource.Id.opportunityTitle);
        public TextView OpportunityType => FindViewById<TextView>(Resource.Id.infotxt_typeOpportunity);
        public TextView CompanyName => FindViewById<TextView>(Resource.Id.infotxt_companyname);
        public TextView PayAmount => FindViewById<TextView>(Resource.Id.infotxt_payAmount);
        public TextView DueDate => FindViewById<TextView>(Resource.Id.infotxt_dueDate);
        public TextView Description => FindViewById<TextView>(Resource.Id.infotxt_description);
        public TextView Requirements => FindViewById<TextView>(Resource.Id.infotxt_requirements);
        public TextView Email => FindViewById<TextView>(Resource.Id.infotxt_email);
        public TextView Address => FindViewById<TextView>(Resource.Id.infotxt_address);
        public TextView PhoneNumber => FindViewById<TextView>(Resource.Id.infotxt_pnumber);
        public TextView AboutTheCompany => FindViewById<TextView>(Resource.Id.infotxt_aboutcompany);
        ...
        public LinearLayout CustomLinear => FindViewById<LinearLayout>(Resource.Id.customLayout);

        public LinearLayout InfoTrainingLinear => FindViewById<LinearLayout>(Resource.Id.linear_info);

        //Rating
        public Button RatingButton => FindViewById<Button>(Resource.Id.btnRating);

        public RatingBar RatingComment => FindViewById<RatingBar>(Resource.Id.ratingBar1);

        public LinearLayout RatingLinearComment => FindViewById<LinearLayout>(Resource.Id.LinearRatingComment);

        public LinearLayout RatingLinearView => FindViewById<LinearLayout>(Resource.Id.RatingLinearView);

        public RatingBar RatingReview => FindViewById<RatingBar>(Resource.Id.ratingreview);

        public TextView lblStarCount => FindViewById<TextView>(Resource.Id.lblStarCount);

        public TextView lblRatingComment => FindViewById<TextView>(Resource.Id.lblRatingComment);

        public EditText EditRatingText => FindViewById<EditText>(Resource.Id.EditRatingComment);
        public ImageView BtnTokSmileyRating => FindViewById<ImageView>(Resource.Id.btnTokInfoSmileyRating);
        public Button BtnRatingComment => FindViewById<Button>(Resource.Id.BtnRatingComment);

        public RecyclerView TokinfoRecyclerTokMojisRating => FindViewById<RecyclerView>(Resource.Id.tokinfoRecyclerTokMojisRating);

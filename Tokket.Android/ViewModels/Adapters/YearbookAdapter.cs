...
{
    public class YearbookAdapter : RecyclerView.Adapter, View.IOnTouchListener, View.IOnLongClickListener
    {
        #region Members/Properties
        // Event handler for item clicks:
        public event EventHandler<int> ItemClick;
        public List<YearbookTok> items;
        public List<YearbookTok> itemsCheckedList;
        ...
        #region Constructor
        public YearbookAdapter(List<YearbookTok> _items) //If caller is Activity or Fragment
        {
            itemsCheckedList = new List<YearbookTok>();
            isShowCheckBox = false;
            items = _items;
        }
        #endregion
        public YearbookAdapter(Context context)
        {
        ...
        public void UpdateItems(List<YearbookTok> listUpdate, int position)
        {
            items.AddRange(listUpdate);
            NotifyItemRangeChanged(position, listUpdate.Count);
        }

        public void AddItem(YearbookTok item)
        {
            items.Add(item);
            NotifyDataSetChanged();
        }

        public void RemoveItem(YearbookTok item)
        {
            items.Remove(item);
            NotifyDataSetChanged();
        }
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
        {
            ...
            if (items[position].YearbookTileType == "Tok")
            {
                vh.gridTokImage.Visibility = ViewStates.Gone;
                if (!string.IsNullOrEmpty(items[position].Image))
                {
                    if (items[position].Image.Contains("data:image/jpeg;base64,"))
                    {
                        byte[] imageDetailBytes = Convert.FromBase64String(items[position].Image.Replace("data:image/jpeg;base64,", ""));
                    ...
                    else
                    {
                        ...
                        Glide.With(this.context).Load(items[position].Image).Thumbnail(0.05f).Apply(options).Into(vh.TokImgMain);
                    }
                }
            }
            else {
                ...
                if (!string.IsNullOrEmpty(items[position].Image))
                {
                    ...
                    {
                        byte[] imageDetailBytes = Convert.FromBase64String(items[position].Image.Replace("data:image/jpeg;base64,", ""));
                        vh.ImageYearbook.SetImageBitmap((BitmapFactory.DecodeByteArray(imageDetailBytes, 0, imageDetailBytes.Length)));
                    }
                    else
                    {
                        RequestOptions options = new RequestOptions()
                        ...
                        Glide.With(this.context).Load(items[position].Image).Thumbnail(0.05f).Apply(options).Into(vh.ImageYearbook);
                    }
                }


                vh.ImageYearbook.Click -= delegate { };  //Add to avoid showing multiple popup
                vh.ImageYearbook.Click += delegate
                {
                    //Add try catch to avoid a possible crash
                    try
                    {
                        Bitmap imgTokMojiBitmap = ((BitmapDrawable)vh.ImageYearbook.Drawable).Bitmap;
                        ...
                        Intent nextActivity = new Intent(this.context, typeof(DialogImageViewerActivity));
       ...
       //     vh.txtLinkImage.SetOnTouchListener(this);
            ...
            if (!string.IsNullOrEmpty(items[position].Image))
            {
                ...
                vh.TokImgTokType.Text = items[position].YearbookSchoolname;
            ...
            else
            {
                ...
                vh.TokType.Text = items[position].YearbookSchoolname;
        ...
        {
            itemView = LayoutInflater.From(parent.Context).Inflate(Resource.Layout.yearbook_tile_row, parent, false);

            vh = new YearbookHolder(itemView, OnClick);
            return vh;
        }
        #endregion
